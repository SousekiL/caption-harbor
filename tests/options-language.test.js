...
test("Settings copy covers English and Simplified Chinese", () => {
  assert.equal(options.translate("en", "pageTitle"), "Caption Harbor Settings");
  assert.equal(options.translate("zh-CN", "pageTitle"), "Caption Harbor 设置");
...
test("customization prompt switches languages and preserves technical values", () => {
  ...
  assert.match(chinesePrompt, /^请把当前本地 Caption Harbor 工作区改为使用/);
