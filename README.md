...
Run `node --test tests/minitool.test.js`, `npx playwright test tests/browser/minitool.spec.js`, then `npm run package:minitool`. The output is `dist/caption-harbor-xiaohongshu-v1.2.1.zip` with `index.html`, `style.css`, `core.js`, `app.js` and `ch.svg` at the root. 1.2.1 adds six clickable offline demo words with meaning/concept explanations, manual IPA, part of speech and examples, the CH icon, and the Chrome Web Store address in the usage notes (no outbound link). Platform approval and Android 8.1 / WebView 61 device compatibility are not yet verified.
