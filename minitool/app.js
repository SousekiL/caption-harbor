(function () {
  ...
  var view = 'read', query = '', page = 0, activeCapture = null, returnFocus = null, toastTimer, searchTimer, sequence = 0;
  function $(id) { return document.getElementById(id); }
  function id() { sequence += 1; return Date.now().toString(36) + '-' + sequence + '-' + Math.random().toString(36).slice(2, 8); }
  function initial() {
    return { version: 1, active: 'welcome', words: [], notes: [], docs: [{ id: 'welcome', title: '每天进步一点点 · 示例', read: [], cues: [
  ...
  } catch (e) { warn('无法读取原有本地内容，当前使用临时示例。重要摘录请手动留存。'); }
  ...
  function render() {
    ...
    $('list-title').textContent = { read: '一句一句读', words: '我的生词本', notes: '留下的想法' }[view];
    $('search').placeholder = view === 'read' ? '在字幕里找一句话' : '搜索全部' + (view === 'words' ? '生词' : '笔记');
    ...
    $('list').textContent = '';
    if (!rows.length) $('list').appendChild(el('div', 'empty', query ? '没有找到匹配内容，换个词试试。' : view === 'read' ? '粘贴一段字幕，开始今天的精读。' : view === 'words' ? '在字幕下点“收藏生词”，留下你的第一个词。' : '在字幕下点“记笔记”，留下原句和自己的想法。'));
    ...
    rows.slice(page * PAGE, (page + 1) * PAGE).forEach(function (entry) {
      ...
      var text = el(view === 'words' ? 'h3' : 'p', view === 'words' ? 'card-title' : 'card-text'); highlight(text, r.text || '原句摘录'); card.appendChild(text);
      if (view !== 'read') {
  ...
  function openModal(kind, title) {
    ...
    ['import', 'capture', 'export', 'about'].forEach(function (name) { $(name + '-panel').hidden = name !== kind; });
    $('dialog-title').textContent = title; $('dialog-error').textContent = ''; $('modal').hidden = false;
    document.body.style.overflow = 'hidden';
    document.querySelector('.sheet').focus();
  }
  function closeModal() { $('modal').hidden = true; activeCapture = null; document.body.style.overflow = ''; if (returnFocus && document.body.contains(returnFocus)) returnFocus.focus(); }
  function capture(type, index, card) {
    ...
    openModal('capture', type === 'word' ? '收藏一个词' : '记下这一句');
    $('capture-context').textContent = cue.text;
    $('word-fields').hidden = type !== 'word'; $('note-fields').hidden = type !== 'note';
    $('word-input').value = selected; $('word-meaning').value = ''; $('note-input').value = '';
  ...
  $('capture-submit').addEventListener('click', function () {
    ...
    if (existing) { existing.meaning = $('word-meaning').value.trim(); }
    else collection.push({ id: id(), docId: activeCapture.docId, title: activeCapture.title, index: activeCapture.index, context: activeCapture.context, start: activeCapture.start, text: text, meaning: word ? $('word-meaning').value.trim() : '', created: new Date().toISOString() });
    save(); closeModal(); render(); toast(existing ? '已更新这个词的释义' : word ? '生词已收藏' : '笔记已保存');
  });
  $('import-open').addEventListener('click', function () { openModal('import', '把字幕带进来'); $('import-title').focus(); });
  ...
  $('export-open').addEventListener('click', function () {
    var lines = ['字幕小港 · 我的学习摘录', ''];
    [['words', '生词'], ['notes', '笔记']].forEach(function (group) {
      lines.push('【' + group[1] + '】');
      state[group[0]].forEach(function (r) { lines.push(r.text || '原句摘录'); if (r.meaning) lines.push('释义：' + r.meaning); lines.push('原句：' + r.context, '来源：' + r.title + (Core.clock(r.start) ? ' · ' + Core.clock(r.start) : ''), ''); });
    });
