# Caption Harbor 2.1.9 完整版

2.1.8 修复关闭弹窗后划词菜单残留，新增菜单关闭按钮与 Esc 收起。

包含词典式词义卡片，包含音标、加粗词性和带来源标签的例句。

解释语言设置：设置 → 学习设置 → 查词与解释，选择“纯英文解释”或“中文解释（英文辅助）”，自动保存。
...
**要使用 2.1.9 新增的本机 Groq／DeepSeek 密钥读取，请更新本机助手。** 助手代码是复制安装的，仅重新加载浏览器扩展不会更新它；已有模型和私有配置可保留。
...
密钥可填写在设置里；DeepSeek／Groq 栏留空时，可从本机私有配置读取 `DEEPSEEK_API_KEY`／`GROQ_API_KEY`。没有附赠 API 额度或托管服务，选择的云端服务可能收费。Apple Podcasts 应打开具体单集；部分单集需要音频转录。网络、站点限制和第三方服务状态仍可能影响获取。
